...
- Telemetry rule: use `./tools/codex-usage probe-sources` to find local telemetry roots, then `ingest-jsonl` or `ingest-state-sqlite` to build the ledger. Treat `sqlite_state` as a discovery signal and `rollout_jsonl` as the likely ingest target; repeated imports are safe because duplicate events are skipped. If you only want the most recent live workers, add `--min-created-at-ms` or `--min-updated-at-ms` so the long parent thread does not drown them out. If you feed the tracker back into Codex, prefer `efficiency-advice` and check `overhead-report` first. The old governor spike is archived under `archive/governor-spike-20260420/`.
