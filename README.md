...
6. If you want the whole tracker loop as one command, use `./tools/codex-usage-checkpoint snapshot` for a repo-wide checkpoint, `./tools/codex-usage-checkpoint mark` before launching agents, and `./tools/codex-usage-checkpoint window` after the agent batch. `window` keeps the main project ledger fresh but emits its report from a separate scoped ledger for that cutoff, and that scoped ledger is rebuilt on each run so stale window data does not leak forward. Re-running `window` with the same cutoff is cumulative since that mark, not a fresh step-local slice; run `mark` again when you want a new baseline. The default `created` mode is the clean child-session slice; use `--cutoff-mode updated` when you want post-mark activity from the already-running parent thread as well.
